...
- **`mcmurphy.exe` round-trip — first post-loop byte divergence (diagnosed
  2026-08-01).** The fixed source now compiles, but the rebuilt image is
  123,920 bytes versus the original 129,710. Comparing scanned operation
  streams finds the first structural difference at operation 133: the input
  has `INT 8A` represented as `stack_call_runtime`, while the rebuilt image
  has only `RET`. The current scanner deliberately treats the six-byte INT 8A
  protocol as source-less because the multi-toggle payload is not a BASIC
  GOSUB target; this prevents the old impossible-target failure, but also
  omits compiler-generated checked-call helper bytes. The next fix needs an
  oracle probe compiled with the same `KBOS` toggles to establish whether this
  protocol is regenerated by a source-level construct or must be modeled as a
  preserved helper region. Do not reinterpret its payload as a GOSUB without
  that calibration.
